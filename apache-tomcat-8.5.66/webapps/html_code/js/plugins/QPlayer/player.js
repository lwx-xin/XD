(function($){
	...
	$("#playlist").on("click","li strong",function(){
		var i = $(this).parent("li").prevAll().length;
		if (isShuffle) {
	...
	$("#playlist").on("click",".remove-music",function(){
	...
	$.addMusic = function(item){
		playlist.push(item);
		$('#playlist').append('<li class="lib" style="overflow:hidden;"><strong style="margin-left: 5px;">'+item.title+'</strong><span class="artist">--'+item.artist+'</span><span style="float: right;" class="remove-music" title="删除"><i class="fa fa-close"></i></span></li>');
		if (item.mp3 == "") {
			$('#playlist li').eq(i).css('color', '#ddd');
		}
...
function showNotification(info) {
	showInformation("播放器提示", info, "info");
